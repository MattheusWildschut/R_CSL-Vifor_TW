...
## TW: Data downloaded from: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE185948

setwd("C:/Users/mwildschut/OneDrive - Vifor Pharma AG/Documents/R/Projects/R_CSL-Vifor_TW/ADPKD_snRNAseq/")

snRNAseq_preprocess = function(input.name, output.name){
  toc = Read10X_h5(paste0("Input/Filtered_Features/", input.name, "_filtered_feature_bc_matrix.h5"))
  tod = Read10X_h5(paste0("Input/Raw_Features/", input.name, "_raw_feature_bc_matrix.h5"))
  ...
  doubletdata <- select(control_1@meta.data, contains("DF.classifications"))
  ...
  saveRDS(control_1, paste0("Input/RDS_Files/", output.name, ".rds"))
}
list.input = as.list(paste0("GSM5627", 690:702, "_", c(paste0("cont", 1:5), paste0("PKD", 1:8))))
list.output = as.list(c(paste0("control_", 1:5), paste0("PKD_", 1:8)))
library(purrr)
map2(list.input, list.output, snRNAseq_preprocess)

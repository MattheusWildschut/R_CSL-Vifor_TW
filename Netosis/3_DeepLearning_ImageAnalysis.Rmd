...
if(save.crops){
  ...
  crop.folder = paste0(folder, "Cell_Crops3/")
...
data = fread(paste0(folder, "Classification/", filename)) %>%
  # filter(Well %in% c("C6", "C12"))# %>%
  group_by(Image, Timepoint) %>% slice_sample(n = 100)

if(save.crops){
  in.folder = paste0(folder, "Raw_Images/")
  out.folder = paste0(folder, "Test_Data2/")
...
### Prepare dataframe for Shiny
```{r Second level CleanUp data prep}
...
classes_c = sort(c("Cell", "NoCell"))
data = fread(paste0(folder, "Classification/", filename))
class_pred_c = model_big_cleanup2 %>% predict(data_pred) %>% ## Repeated run! #model_big_cleanup %>% predict(data_pred) %>%
  as.data.frame %>%
  dplyr::mutate(Class = apply(., 1, function(z) which(z == max(z))),
                Class = factor(Class, levels = 1:length(classes_c), labels = classes_c),
                Filename = str_remove_all(data_pred$file_paths, ".*\\\\|Netosis_Exp.._|.png"),
                Class2 = NA,
                ObjectNumber = as.numeric(str_remove(Filename, ".*-")),
                Filename2 = str_remove_all(Filename, "-.*")) %>%
  left_join(data[,1:12], by = c("Filename2", "ObjectNumber")) %>%
  dplyr::select(Filename, Filename2, X, Y, Class, Class2)

write.csv(class_pred_c, paste0(folder, "Classification/Netosis_", exp_name, "_CleanUpClassification3.csv"), row.names = FALSE)

```

### Store curated image crops & into folders
```{r 2nd cleanup curated images}
filename2 = "Netosis_Exp17_CleanUpClassification3_2023-08-29_15-21-13.csv"
folder_train_c2 = paste0(folder_cleanup2, "TrainingData3/")

if(save.crops){
  set.seed(123)
  data = fread(paste0(folder, "Classification/", filename2)) %>%
    filter(!is.na(Class2)) %>%
    group_by(Class2) %>% add_tally(name = "Cells") %>% dplyr::slice_sample(n = min(.$Cells))

  n.cores <- parallel::detectCores() - 1
  doSNOW::registerDoSNOW(cl = snow::makeSOCKcluster(n.cores))
  
  in.folder = paste0(folder, "Raw_Images/")
  if(!dir.exists(folder_train_c2)) dir.create(folder_train_c2)
  crop.size = 50
  
  progress <- function(n) setTxtProgressBar(txtProgressBar(max = length(unique(data$Filename2)), style = 3), n)
  opts <- list(progress = progress)

  for(class in unique(data$Class2)){
    if(!dir.exists(paste0(folder_train_c2, class))) dir.create(paste0(folder_train_c2, class), recursive = TRUE)
    data.class = data %>% filter(Class2 == class)
    foreach(i=1:nrow(data.class), .options.snow = opts) %dopar% {
      img.comb = array(dim = c(50,50,3), dimnames = list(NULL, NULL, c("Phase", "Green", "Red")))
      for(channel in c("Green", "Phase", "Red")){
        filename = paste(exp_name, channel, data.class$Filename2[i], sep = "_")
        img_arr = keras::image_to_array(keras::image_load(paste0(in.folder, "Netosis_", filename, ".png"), color_mode = "grayscale"))
        img_idx = c("X" = ceiling(data.class$X[i]), "Y" = ceiling(data.class$Y[i]))
        img.comb[,,channel] = img_arr[img_idx[2]:(img_idx[2]+50-1), img_idx[1]:(img_idx[1]+50-1),1]
      }
      keras::image_array_save(path = paste0(folder_train_c2, class, "/", 
                                            data.class$Filename2[i], "_", stringr::str_extract(data.class$Filename[i], "(?<=-).*"), "-", class, "_", i, ".png"), 
                              img = img.comb, scale = FALSE)
    }
    print(paste0("Class ", class, ": ", nrow(data.class), " images saved"))
  }
}
...
train_data_gen <- image_data_generator(# featurewise_center = TRUE,
                                       ...
                                       validation_split = 0.2)#, # 20% data as validation data
                                       #channel_shift_range = 5)
train_image_array_gen <- flow_images_from_directory(directory = folder_train_c2, # Data folder
                                                    target_size = target_size, # target of the image dimension
                                                    color_mode = "rgb", # use RGB color
                                                    batch_size = batch_size, 
                                                    seed = 123,  # set random seed
                                                    subset = "training", # declare that this is training data
                                                    generator = train_data_gen)
val_image_array_gen <- flow_images_from_directory(directory = folder_train_c2,
                                                  target_size = target_size, 
                                                  color_mode = "rgb", 
                                                  batch_size = batch_size,
                                                  seed = 123,
...
model_big_cleanup2 <- keras_model_sequential() %>%
...
if(train.model){
  set.seed(123)
  if(!dir.exists(folder_model_c2)) dir.create(folder_model_c2)
  # ResNet = application_vgg16(classes = 6, weights = NULL, input_shape = c(50,50,3), pooling = "avg")
  ...
  # history <- ResNet %>% 
  history <- model_big_cleanup2 %>%
    fit(train_image_array_gen, 
        steps_per_epoch = as.integer(train_samples / batch_size), epochs = 100, 
        validation_data = val_image_array_gen, validation_steps = as.integer(valid_samples / batch_size),
        callbacks = list(
          callback_model_checkpoint(paste0(folder_model_c2, "CleanUp_weights_{epoch:02d}-{val_accuracy:.2f}_", 
                                           format(Sys.time(), "%Y-%m-%d_%H-%M-%S"), ".hdf5"), 
                                    monitor = "val_accuracy", verbose = 1, save_best_only = TRUE),
          callback_reduce_lr_on_plateau(monitor = "val_accuracy", factor = 0.25, patience = 10)
        ))
  # pdf(paste0(folder, "Models/Training_CellNoCell_", format(Sys.time(), "%Y-%m-%d_%H-%M-%S"), ".pdf"), width = 9, height = 6)
  # plot(history)
  # dev.off()
...
### Save validation images
```{r Validation images}
folder_val_c2 = paste0(folder_cleanup2, "ValidationData")
if(!dir.exists(folder_val_c2)) dir.create(folder_val_c2)

invisible(foreach(i=val_image_array_gen$filepaths, .packages = "stringr") %dopar%{
  file.copy(i, str_replace(i, "TrainingData\\\\.*\\\\", "ValidationData/"))
})

val_data = image_dataset_from_directory(directory = folder_val_c2,
                                        image_size = c(50, 50),
                                         color_mode = "rgb",
                                         label_mode = NULL,
                                         shuffle = FALSE)
...
# load_model_weights_tf(model_big_cleanup2, weights_path[length(weights_path)])
classes = sort(c("Cell", "NoCell"))
classes2 = c("Cell", "NoCell", "Overall")

val_pred = predict(model_big_cleanup2, val_data) %>%
  as.data.frame %>%
  dplyr::mutate(Predict = apply(., 1, function(z) which(z == max(z))),
                Predict = factor(Predict, levels = 1:length(classes), labels = classes),
                Curate = factor(str_extract(val_data$file_paths, "(?<=-).*(?=_)"), levels = classes))
...
ggplot(conf.plot, aes(x = Reference, y = Prediction, fill = Accuracy)) +
  ...
  theme_classic() + theme(axis.line = element_blank(), axis.ticks = element_blank()) +
  coord_cartesian(expand = FALSE)
ggsave(paste0(folder_model_c2, "ConfusionMatrix", format(Sys.time(), "%Y-%m-%d_%H-%M-%S"), ".pdf"), width = 4, height = 2.75)
ggsave(paste0(folder_model_c2, "ConfusionMatrix", format(Sys.time(), "%Y-%m-%d_%H-%M-%S"), ".png"), width = 4, height = 2.75)

```


## Cell type classifier
### Save crops for curation
```{r Cell type curation crops}
folder_class = paste0(folder, "CNN_CellType/")
data = fread(paste0(folder, "Classification/", filename2)) %>%
    filter(Class2 == "Cell")

folder_curation = paste0(folder_class, "Test_Data2/")
if(!dir.exists(folder_curation)) dir.create(folder_curation, recursive = TRUE)
invisible(foreach(i=data$Filename, .packages = "stringr") %dopar%{
  path = paste0(folder, "Test_Data2/Netosis_", exp_name, "_", i, ".png")
  file.copy(path, str_replace(path, folder, folder_class))
})

folder_classification = paste0(folder_class, "Classification/")
if(!dir.exists(folder_classification)) dir.create(folder_classification)
data.orig = fread(paste0(folder, "Classification/", filename))
data_curation = data %>% 
  dplyr::mutate(ObjectNumber = str_extract(Filename, "(?<=-).*"),
                ImageNumber = str_extract(Filename, "(?<=[A-Z][:digit:]{1,2}_)."),
                Class = NA) %>% dplyr::select(-Class2)
write.csv(data_curation, paste0(folder_classification, "CellTypeClassification.csv"))
...
### Merge channels & organize in folders by class
```{r Merge channels & class folders}
data_class = fread(paste0(folder_class, "Classification/CellTypeClassification_2023-08-30_11-58-55.csv")) %>%
  filter(!is.na(Class))
...
#     crop.folder = paste0(folder, "Raw_Images/")
#     img.comb = array(dim = c(50,50,3), dimnames = list(NULL, NULL, c("Phase", "Green", "Red")))
#     for(i in 1:nrow(data.crops)){
#       for(channel in c("Phase", "Green", "Red")){
#         img.comb[,,channel] = image_to_array(image_load(paste0(crop.folder, "Netosis_", exp_name, "_", channel, "_", 
...
if(save.crops){
  in.folder = paste0(folder, "Raw_Images/")
  out.folder = paste0(folder_train, unique(data_class$Class))
  if(!all(dir.exists(out.folder))) walk(as.list(out.folder), dir.create, showWarnings = FALSE, recursive = TRUE)
  ...
  # images = foreach(file = unique(data_class$Filename2), .combine = c) %do% {which(file == data$Filename2)[1]}
  
  invisible(foreach(x=1:nrow(data_class), .packages = "keras", .options.snow = opts) %dopar%{
    # for(object in which(data$Filename2 == data$Filename2[i])){
      img.comb = array(dim = c(936,1264,3), dimnames = list(NULL, NULL, c("Phase", "Green", "Red")))
      for(channel in c("Phase", "Green", "Red")){
        img.name = paste("Netosis", exp_name, channel, data_class$Filename2[x], sep = "_")
        img = image_load(paste0(in.folder, img.name, ".png"), color_mode = "grayscale")
        img.comb[,,channel] = image_to_array(img)
      }
      img.crop = img.comb[floor(data_class$Y[x]):floor(data_class$Y[x]+crop.size-1),
                          floor(data_class$X[x]):floor(data_class$X[x]+crop.size-1),]
      image_array_save(path = paste0(folder_train, data_class$Class[x], "/", exp_name, "_", data_class$Filename2[x],
                                     "-", data_class$ObjectNumber[x], ".png"), 
                     img = img.crop, scale = FALSE)
    # }
  })
}
...
if(train.model){
  ...
  history <- model_big %>%
    fit(train_image_array_gen, 
        ...
        validation_data = val_image_array_gen, validation_steps = as.integer(valid_samples / batch_size),
        callbacks = list(
          callback_model_checkpoint(paste0(folder_model, "CellType-Weights_", format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
                                           "_{epoch:02d}-{val_accuracy:.2f}.hdf5"), 
                                    monitor = "val_accuracy", verbose = 1, save_best_only = TRUE),
          callback_reduce_lr_on_plateau(monitor = "val_accuracy", factor = 0.1, patience = 10)
        ))
  plot(history)
...
folder_val_classes = paste0(folder_val, 
                            unlist(unique(data_class %>% filter(Class!= "NoCell") %>% dplyr::select(Class))))
if(!all(dir.exists(folder_val_classes))) walk(as.list(folder_val_classes), dir.create, showWarnings = FALSE, recursive = TRUE)
...
classes = sort(c("Adherent", "Flat", "NET", "Round"))
classes2 = c("Round", "Adherent", "Flat", "NET", "Overall")

val_pred = predict(model_big, val_data) %>%
  as.data.frame %>%
  dplyr::mutate(Predict = apply(., 1, function(z) which(z == max(z))),
                Predict = factor(Predict, levels = 1:length(classes), labels = classes),
                Curate = factor(str_extract(val_data$file_paths, paste(classes, collapse = "|")), levels = classes),
                Filename = str_remove_all(val_data$file_paths, ".*Exp17_|.png"))
...
# i = 3
  ...
  # for(i in 1:8){
    pred_test = foreach(x = data_list, .combine = rbind.data.frame, .multicombine = TRUE, 
...
img.comb = array(dim = c(936,1264,3), dimnames = list(NULL, NULL, c("Phase", "Green", "Red")))
      for(channel in c("Phase", "Green", "Red")){
        img.name = paste("Netosis", exp_name, channel, "A9_2_00d04h30m", sep = "_")
        img = image_load(paste0(in.folder, img.name, ".png"), color_mode = "grayscale")
        img.comb[,,channel] = image_to_array(img)
      }
      img.crop = img.comb[273:(273+49),819:(819+49),]
      image_array_save(path = paste0(folder_class, "Trial_Crop6.png"), 
                     img = img.crop[,,2,drop=FALSE], scale = FALSE)

      
      a = magick::image_read(img.crop, coalesce = FALSE)

png(paste0(folder_class, "Trial_Crop7.png"), width = 50, height = 50)
magick::image_draw(image_read(img.crop))
dev.off()
      
a = keras::image_to_array(
  keras::image_load("C:/Incucyte/Netosis/Netosis_Exp17/CNN_CellType/Trial_Crop7.png", color_mode = "grayscale"))

d = keras::image_load("C:/Incucyte/Netosis/Netosis_Exp17/CNN_CellType/Trial_Crop3.png")
keras::ima

a = keras::image_to_array(
  keras::image_load("C:/Incucyte/Netosis/Netosis_Exp17/CNN_CellType/TrainingData/NET/Exp17_A9_2_00d04h30m-92.png"))

write.csv(img.crop, "C:/Incucyte/Netosis/Netosis_Exp17/CNN_CellType/Trial_Crop4.csv")

a = magick::image_read("C:/Incucyte/Netosis/Netosis_Exp17/CNN_CellType/Trial_Crop5.png")
magick::image_write(a, path = paste0(folder_class, "Trial_Crop5.png"))
a = as.integer(a[[1]])

img.array = array(dim = c(1,50,50,3))
img.array[1,,,] = img.crop
predict(model_big, img.crop)

a = imager::load.image("C:/Incucyte/Netosis/Netosis_Exp17/CNN_CellType/Trial_Crop4.png")

z = as.data.frame(a[,,2])
y = as.data.frame(img.crop[,,2])
q = as.data.frame(img.crop[,,3])

x$Green
x$Red

p = as.data.frame(image_to_array(image_load(x[["Green"]], color_mode = "grayscale")))
p = as.data.frame(p[273:(273+49),819:(819+49)])
str(p)

predict(model_big, img.array)
...
#                                          shuffle = FALSE)
# classes_c = sort(c("Cell", "NoCell"))
# class_pred_c = model_big_cleanup2 %>% predict(data_pred) %>%
#   as.data.frame %>%
#   dplyr::mutate(CleanUp = apply(., 1, function(z) which(z == max(z))),
#                 CleanUp = factor(CleanUp, levels = 1:length(classes_c), labels = classes_c),
#                 Filename = str_remove_all(data_pred$file_paths, ".*\\\\|Netosis_Exp.._|.png"))
# 
# sel_pred_c = class_pred_c %>%
#   dplyr::mutate(Image = str_remove(Filename, "-.*")) %>%
#   group_by(Image, CleanUp) %>% slice_sample(n = 50)

# tSNE_fit = Rtsne(-log10(sel_pred_c[,1:length(classes_c)]))
# tSNE_df = tSNE_fit$Y  %>%
#   as.data.frame %>%
#   `colnames<-`(c("X", "Y")) %>%
#   mutate(Prediction = sel_pred_c$CleanUp)#,
#          # Curation = str_extract(class_pred$Filename, paste(names(data_pred$class_indices), collapse = "|")))

classes_c = sort(c("Cell", "NoCell"))
classes = sort(c("Adherent", "Flat", "NET", "Round"))

platemap = fread(paste0(folder, exp_name, "_PlateMap.csv"))
data_all = data_all %>%
  dplyr::mutate(Well = str_extract(Filename, "^[A-H][:digit:]{1,2}"),
                Image = str_extract(Filename, "[:digit:](?=_[:digit:]{2}d[:digit:]{2}h[:digit:]{2}m)"),
                Timepoint = str_extract(Filename, "[:digit:]{2}d[:digit:]{2}h[:digit:]{2}m"),
                Condition = platemap$PMA[match(Well, platemap$Well)],
                Cleanup = ifelse(Cell > NoCell, "Cell", "NoCell"),
                Celltype = classes[apply(.[,..classes], 1, function(z) which(z == max(z)))])

table(data_all$Celltype)

set.seed(123)
sel_pred = data_all %>%
  filter(Cleanup == "Cell") %>%
  group_by(Well, Image, Celltype) %>% slice_sample(n = 50) %>% ungroup
nrow(unique(data_all[,c("Well", "Image")]))*50
nrow(unique(data_all[,c("Well", "Timepoint")]))*4*5
nrow(unique(data_all[,c("Condition", "Timepoint")]))*4


data_tsne = sel_pred %>%
  dplyr::select(all_of(classes), Cleanup) %>%
  dplyr::mutate(across(all_of(classes_c), ~ -log10(.x))) %>%
  dplyr::mutate(across(all_of(classes_c),
                       ~ ifelse(is.infinite(.x),
                                max(.x[!is.infinite(.x)])*runif(length(is.infinite(.x)), 0.98, 1.02), .x))) %>%
  filter(!duplicated(.[,classes_c]))
tSNE_fit = Rtsne(data_tsne[,classes_c])

tSNE_df = tSNE_fit$Y  %>%
  as.data.frame %>%
  `colnames<-`(c("X", "Y")) %>%
  mutate(CleanUp = data_tsne$Cleanup)

ggplot(tSNE_df, aes(x = X, y = Y, col = CleanUp)) +
...
class_pred = model_big %>% predict(data_pred) %>%
  ...
  dplyr::mutate(Class = apply(., 1, function(z) which(z == max(z))),
                ...
                Filename = str_remove_all(data_pred$file_paths, ".*\\\\|Netosis_Exp.._|.png")) %>%
  cbind.data.frame(class_pred_c %>% select(CleanUp))

sel_pred = class_pred %>%
  dplyr::mutate(Image = str_remove(Filename, "-.*")) %>%
  filter(CleanUp != "NoCell") %>%
...
img.sel = "C6_1_00d06h00m"
img = image_read(paste0(folder, "MultiChannel_Images/Netosis_", exp_name, "_Merged_", img.sel, ".jpg"))
img = image_draw(img)
data.sel2 = data  %>% filter(Filename2 == img.sel)
data.sel = data.annot %>% filter(Filename2 == img.sel) %>%
  mutate(Class = ifelse(CleanUp == "NoCell", "NoCell", as.character(Class)))

...
plots.all = ggplot(data, aes(x = Group, y = `Intensity (Normalized)`, fill = Group, group = Group)) +
  geom_boxplot(linewidth = 0.2, outlier.size = 0) + geom_jitter(width = 0.1, alpha = 0.5, size = 0.5) +
  facet_grid(Tissue ~ Metabolite, scales = "free") +
  stat_compare_means(comparisons = list(c(1,2), c(1,3), c(2,3)), method = "t.test", method.args = list(var.equal = TRUE), label = "p.signif", hide.ns = TRUE, vjust = 0.4) +
...
plot.udpgluc = ggplot(filter(data, Tissue == "Kidney" & Metabolite == "UDP.D\nglucose"), aes(x = Group, y = `Intensity (Normalized)`, fill = Group, group = Group)) +
  geom_boxplot(linewidth = 0.2, outlier.size = 0) + geom_jitter(width = 0.1, alpha = 0.5) +
  facet_grid(Tissue ~ Metabolite, scales = "free") +
stat_compare_means(comparisons = list(c(1,2), c(1,3), c(2,3)), method = "t.test", method.args = list(var.equal = TRUE), label = "p.signif", hide.ns = TRUE, vjust = 0.4) +
  theme_bw() + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())

plots.all + (plot_spacer() / plot.udpgluc / plot_spacer() + plot_layout(heights = c(1,2,1))) + 

...
data = raw.data %>%
  ...
  dplyr::mutate(across(contains(c("/dL", "/mL", "/L")), as.numeric),
                ...
                # `Baseline eGFR (ml/min/1.73m2) (Binned)` = str_remove_all(`Baseline eGFR (ml/min/1.73m2) (Binned)`,
                                                                                     # "-.*|\\[|,.*"))

vars = list("KDIGO Stage")#, "Baseline eGFR (ml/min/1.73m2) (Binned)")
plot.list = map(vars, function(var){
  data.plot = data %>% filter(Disease == "AKI")
  ggplot(data.plot, aes(x = .data[[var]], y = `Calcium (mg/dL) * Phosphate (mg/dL)`, fill = .data[[var]], col = .data[[var]])) +
    geom_boxplot(alpha = 0.25) +
    geom_jitter(alpha = 0.5, width = 0.2, size = 1) +
    stat_compare_means(comparisons = list(c(2,3)), method = "t.test", method.args = list(var.equal = TRUE), size = 3) +
    theme_bw()
})

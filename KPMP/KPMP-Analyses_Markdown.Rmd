...
```{r CaP correlations, fig.width=5.75, fig.height=6.25}
...
plot.list = map(cohorts, function(cohort){
  map2(vars, cohort, function(var, cohort){
    ...
    if(cohort == "All") plot = ggplot(data.plot, aes(x = Calcium_x_Phosphate, y = .data[[var]],
                                                     col = Disease))
    if(cohort == "AKI") plot = ggplot(data.plot, aes(x = Calcium_x_Phosphate, y = .data[[var]]))
    plot +

...
### Transplants Roedder 2013

Paper: Roedder S, Kimura N, Okamura H, Hsieh SC et al. Significance and suppression of redundant IL17 responses in acute allograft rejection by bioinformatics based drug repositioning of fenofibrate. PLoS One 2013;8(2):e56657.
Link: https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0056657
Data from: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE34437

```{r Roedder transplant kidney, fig.width=13, fig.height=3}
data = getGEO("GSE34437")[[1]]
annot.pat = data@phenoData@data %>%
  `colnames<-`(str_replace(str_remove(colnames(.), ":ch1"), " ", "_")) %>%
  mutate(Disease = factor(disease_status, levels = unique(disease_status)[c(5,4,3,1,2)])) %>%
  select(geo_accession, Disease) %>%
  arrange(Disease)
...
ggplot(expr.long, aes(x = .data[[var]], y = Expression, fill = .data[[var]], col = .data[[var]])) +
  ...
  facet_wrap(.~Gene, scales = "free", nrow = 1) +
  stat_compare_means(comparisons = list(c(2,3), c(2,4), c(3,4)), na.rm = TRUE, size = 2.5) +
  labs(x = NULL, y = "mRNA levels (log2)", col = "Condition", fill = "Condition", 
       title = "Transplant transcriptomics\nRoedder PLoS One 2013") +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
  theme_bw() + 
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        plot.title = element_text(hjust = 0.5))

ggsave("Output/Transplant-Roedder2013-Transcriptomics_All.pdf", width = 13, height = 3)

```

### Transplants Roedder 2009

Paper: Rödder S, Scherer A, Raulf F, Berthier CC et al. Renal allografts with IF/TA display distinct expression profiles of metzincins and related genes. Am J Transplant 2009 Mar;9(3):517-26. PMID: 19191772
Link: https://www.sciencedirect.com/science/article/pii/S1600613522015994?via%3Dihub
Data from: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE9493

```{r Roedder transplant kidney, fig.width=14, fig.height=3}
data = getGEO("GSE9493")[[1]]
annot.pat = data@phenoData@data %>%
  `colnames<-`(str_replace(str_remove(colnames(.), ":ch1"), " ", "_")) %>%
    mutate(Condition = str_extract(source_name_ch1, "Acute rejection|non-rejecting|borderline rejection|Chronic allograft nephropathy biopsy grade I{1,3}|acute rejection and chronic allograft nephropathy|control kidney")) %>%
  mutate(Condition = factor(Condition, levels = unique(Condition)[c(8,7,3,4,5,6,1,2)])) %>% #,

  #   filter(str_detect(source_name_ch1, "nephropathy"))
  # 
  # 
  # 
  # filter(str_detect(source_name_ch1, "non-rejecting"))
  # 
  # filter(str_detect(source_name_ch1, "Acute rejection|borderline"))
  #        Tissue = str_extract(source_name_ch1, "control kidney|transplanted kidney|kidney allograft")) %>% #, levels = unique(disease_status)[c(5,4,3,1,2)])) %>%
  select(geo_accession, Condition) %>%
  arrange(Condition)

pat.counts = annot.pat %>% group_by(Condition) %>% dplyr::count() %>% ungroup %>%
  mutate(Labels = paste0(Condition, " (", n, ")")) %>%
  mutate(Labels = factor(Labels, levels = Labels))
annot.pat = annot.pat %>% left_join(pat.counts, by = "Condition")
annot.gene = data@featureData@data
expr = data@assayData$exprs %>% as.data.frame %>%
  mutate(Gene = annot.gene$`Gene Symbol`[match(rownames(.), annot.gene$ID)]) %>%
  filter(!is.na(Gene)) %>%
...
gene.list = c("CCL17", "IL3RA", "PLG", "LY96", "CXCR3")
gene.list = c("LY96", "MPO", "NGAL", "PRTN3", "ELANE", "CTSG")
...
expr.long = expr %>%
  dplyr::select(var, all_of(gene.list[gene.list %in% colnames(expr)])) %>%
  pivot_longer(cols = -var, names_to = "Gene", values_to = "Expression") %>%
  mutate(Expression = log2(Expression), Gene = factor(Gene, levels = gene.list))
ggplot(expr.long, aes(x = .data[[var]], y = Expression, fill = .data[[var]], col = .data[[var]])) +
  geom_boxplot(alpha = 0.25, outlier.alpha = 0, col = "black") + geom_jitter(width = 0.2, alpha = 0.25) +
  facet_wrap(.~Gene, scales = "free", ncol = length(gene.list[gene.list %in% colnames(expr)])) +
  labs(x = NULL, y = "mRNA levels (log2)", col = "Condition", fill = "Condition", 
       title = "Transplant transcriptomics\nRoedder Am J Transplant 2009") +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
  theme_bw() + 
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        plot.title = element_text(hjust = 0.5))

ggsave("Output/Transplant-Roedder2009-Transcriptomics_All.pdf", width = 14, height = 3)

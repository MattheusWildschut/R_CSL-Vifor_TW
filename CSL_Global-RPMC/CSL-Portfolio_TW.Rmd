...
### NephroSeq Berthier Lupus Glom & TubInt
...
```{r LN Wither, fig.width=7, fig.height=5}
...
plot.list = map2(gene.list[gene.list %in% colnames(expr)], var, function(gene, var){
  ggplot(expr, aes(x = .data[[var]], y = .data[[gene]], fill = .data[[var]], col = .data[[var]])) +
    geom_boxplot(alpha = 0.25, outlier.alpha = 0, col = "black") + geom_jitter(width = 0.2, alpha = 0.25) +
    # facet_wrap(.~Tissue) +
    stat_compare_means(comparisons = list(c(1,2), c(1,3), c(2,3)), na.rm = TRUE, size = 3) +
    labs(x = NULL, y = paste0(gene, "\nmRNA levels (log2)"), fill = "Disease status", col = "Disease status") +
    scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
    theme_bw() + 
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
...
```{r NEPTUNE Glom, fig.width=9.5, fig.height=5}
...
wrap_plots(plot.list, guides = "collect") + 
  plot_annotation("Neptune Glomerular RNAseq\nSealfon JASN 2022") & theme(plot.title = element_text(hjust = 0.5))
# ggsave("Output/IgAN_CoxBlood2_All.pdf", width = 6, height = 4.5)
...
```{r Neptune TI, fig.width=9.5, fig.height=5}
...
wrap_plots(plot.list, guides = "collect") + 
  plot_annotation("Neptune Tubulointerstitial RNAseq") & theme(plot.title = element_text(hjust = 0.5))
# ggsave("Output/IgAN_CoxBlood2_All.pdf", width = 6, height = 4.5)
...
### ERCB subjects across kidney diseases

...
```{r Data Natalia, fig.width=6, fig.height=2.5}
data.proc = readxl::read_excel("Input/Excel-data_qPCR_2023-07-18.xlsx", sheet = "qPCR_Data") %>%
  ...
  # filter(Day2 == "Timepoint 2" & CellType != "TERT1 - Transwell")
...
ggplot(data.sum, aes(x = Gene, y = CellType, fill = log10(FC))) + #size = Baseline, 
  # geom_point(shape = 21, col = "grey80") +
  geom_tile(col = "grey80") +
  geom_text(aes(label = round(FC,1)), col = ifelse(data.sum$FC > 3, "white", "black")) +
  ...
  labs(y = NULL, fill = "Fold change") + scale_y_discrete(limits = rev, position = "right")

ggsave("Output/Heatmap-Poster_qPCR-Natalia_Horizontal_TW.pdf", width = 6, height = 2.5)

```

### TERT data Patrick
```{r TERT data Patrick, fig.width=3.25, fig.height=5}
...
ggplot(data, aes(x = Timepoint, y = Gene, fill = log10(FC))) + #size = Baseline, 
  # geom_point(shape = 21, col = "grey80") +
  geom_tile(col = "grey80") +
...
ggsave("Output/Heatmap-Poster_qPCR-Patrick2_TW.png", width = 3.25, height = 5)

```

### Injury data Natalia
```{r Injury data Natalia, fig.width=4.75, fig.height=3}
conditions = c("Medium", "200uM BSA control", "600uM BSA control", "200uM Palmitate-BSA", "600uM Palmitate-BSA")
genes = c("HAVCR1", "IL6", "CSF3", "CPT1A", "CUBN")
data = readxl::read_excel("Input/Excel-data_qPCR-Injury_2023-07-19.xlsx", sheet = "qPCR_Data") %>%
  ...
  mutate(Condition = factor(Condition, levels = conditions), Gene = factor(Gene, levels = genes))
data.sum = data %>%
  group_by(Condition, Gene) %>% dplyr::summarise(Expression = mean(`Normalized mRNA expression`)) %>%
  mutate(Condition2 = factor(Condition, levels = levels(Condition), labels = rep(c("Medium", "BSA control", "Palmitate-BSA"), c(1,2,2))),
         Condition = str_remove(Condition, " .*")) %>%
  filter(Condition != "Medium")


ggplot(data.sum, aes(x = Gene, y = Condition, fill = log10(Expression))) + #size = Baseline, 
  # geom_point(shape = 21, col = "grey80") +
  geom_tile(col = "grey80") +
  geom_text(aes(label = replace_na(as.character(ifelse(Expression > 99, round(Expression,0), round(Expression,1))), "ND")), 
            col = ifelse(is.na(data.sum$Expression), "black", ifelse(data.sum$Expression > 3, "white", "black"))) +
  scale_fill_gradient2(na.value = "grey90", midpoint = 0, labels = ~round(10^.x,1)) +
  # scale_fill_viridis(na.value = "white") +
...
ggsave("Output/Heatmap-Poster_qPCR-InjuryNatalia_TW.png", width = 4.75, height = 3)
```
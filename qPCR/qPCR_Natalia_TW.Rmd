...
## Experiment RPTEC3
```{r Experiment RPTEC3, fig.width=15, fig.height=5.5}
...
data = rbind.data.frame(data1, data2) %>%
  ...
  filter(Gene != "HPRT") %>% mutate(Gene = factor(Gene, levels = layout.genes)) %>%
  # group_by(Gene) %>% dplyr::mutate(`Normalized mRNA expression` = 2^-(Cp_norm-mean(Cp_norm[Condition == "Medium"], na.rm = TRUE))) %>%
  mutate(Condition2 = factor(replace_na(str_extract(Condition, "Incucyte"), "Not-Incucyte"),
                             levels = c("Incucyte", "Not-Incucyte"))) %>%
  group_by(Gene, Condition2) %>% dplyr::mutate(`Normalized mRNA expression` = 2^-(Cp_norm-mean(Cp_norm[str_detect(Condition, "Medium")], na.rm = TRUE))) %>%
  arrange(Gene, Condition, `Normalized mRNA expression`)

ggplot(data, aes(x = Condition, y = `Normalized mRNA expression`, fill = Condition, col = Condition, shape = Condition2)) +
  geom_hline(yintercept = 1, linetype = 2) +
  geom_point(size = 1.75, col = "grey30", stroke = 0.01) +
  geom_vline(xintercept = nrow(sample.list)/2+0.5, linetype = 2) + 
  facet_wrap(.~Gene, scales = "free") +
  scale_shape_manual(values = c("Incucyte" = 21, "Not-Incucyte" = 23)) +
  scale_fill_manual(values = setNames(rep(hue_pal()(nrow(sample.list)/2), 2), sample.list$Samples)) +
  theme_bw() + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),
                     panel.grid.major.x = element_blank()) +
  xlab(NULL) + scale_y_continuous(trans = "log2") + #limits = c(0, NA), expand = expansion(mult = c(0.03,0.05)), 
  guides(fill = guide_legend(override.aes = list(shape = ifelse(str_detect(unique(data$Condition), "Incucyte"), 21, 23))),
         shape = "none")#guide_legend(override.aes = list(fill = "black")))

ggsave("Output/Natalia-RPTEC3_Log2_Norm2Mediums_TW_16-06-23.pdf", width = 15, height = 5.5)

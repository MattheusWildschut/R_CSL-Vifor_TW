...
data = raw.data %>%
  ...
  mutate(Condition = factor(Condition, levels = c("DOX+", "DOX-")),
         Ion = factor(Ion, levels =  c("Mn55", "Cu63", "Zn66", "Fe58", "Fe56"))) %>%
  filter(!is.na(Ion)) %>%
...
ggplot(data, aes(x = Condition, y = Concentration, group = Condition, col = Condition)) +
  ...
  labs(x = NULL, y = "Ion concentration (ug/L) / Protein (mg/ml)") +
...
```{r RPA data, fig.width=5, fig.height=5.25}
data = fread("Input/RPA-data_TW.csv", stringsAsFactors = TRUE) %>%
